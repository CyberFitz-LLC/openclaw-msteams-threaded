...
export async function resolveMSTeamsSendContext(params: {
  ...
  const { conversationId: storeConversationId, ref } = found;
  // When the target includes ;messageid= (thread reply), use the full conversation ID
  // so the proactive send goes to the specific thread, not the channel root.
  // The store normalizes conversation IDs (strips ;messageid=), but the original target
  // contains the thread-specific suffix we need for delivery.
  const conversationId =
    recipient.type === "conversation" && recipient.id.includes(";messageid=")
      ? recipient.id
      : storeConversationId;
  // Also update the stored ref's conversation.id so buildConversationReference uses the thread ID
  if (
    recipient.type === "conversation" &&
    recipient.id.includes(";messageid=") &&
    ref.conversation
  ) {
    ref.conversation.id = recipient.id;
  }

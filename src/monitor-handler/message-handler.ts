...
export function createMSTeamsMessageHandler(deps: MSTeamsMessageHandlerDeps) {
  ...
  const handleTeamsMessageNow = async (params: MSTeamsDebounceEntry) => {
    ...
    // For thread sessions, use the full conversation ID (with ;messageid=XXXXX) so replies go to the thread
    // Check if thread sessions are enabled and this is a thread message
    ...
    // Thread-based session routing (opt-in via config)
    // When enabled, each thread in a channel/group gets its own session context
    // Teams embeds thread ID in conversation.id as ";messageid=XXXXX" suffix
    const useThreadSessions = !isDirectMessage && msteamsCfg?.threadSessions === true;
    const extractTeamsThreadId = (convId: string | undefined): string | undefined => {
      if (!convId) return undefined;
      const match = convId.match(/;messageid=(\d+)$/);
      return match?.[1];
    };
    const threadId = useThreadSessions
      ? extractTeamsThreadId(activity.conversation?.id)
      : undefined;

    // For thread messages, use the full conversation ID (with ;messageid=) so replies go to the correct thread
    ...
    // Build peer ID - include thread ID if thread sessions are enabled
    ...
    // Log thread routing decisions for debugging
    if (!isDirectMessage) {
      console.log(
        "[THREAD-DEBUG]",
        JSON.stringify({
          threadSessionsEnabled: useThreadSessions,
          replyToId: activity.replyToId ?? "none",
          threadId: threadId ?? "none",
          peerId,
          conversationType,
          activityId: activity.id,
          // Check other potential thread identifiers
          conversationId: activity.conversation?.id,
          conversationIsGroup: activity.conversation?.isGroup,
          conversationConversationType: activity.conversation?.conversationType,
          conversationTenantId: activity.conversation?.tenantId,
          channelData: JSON.stringify(activity.channelData ?? {}),
        }),
      );
      log.info("thread routing", {
        threadSessionsEnabled: useThreadSessions,
        replyToId: activity.replyToId ?? "none",
        threadId: threadId ?? "none",
        peerId,

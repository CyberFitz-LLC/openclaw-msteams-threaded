...
import type { MSTeamsReplyStyle } from "./types.js";
...
// Note: We preserve the full conversation ID including ;messageid= suffix for thread replies
// The ;messageid= suffix tells Teams to reply within a thread
export function buildConversationReference(
  ...
  return {
    ...
    agent,
    conversation: {
      // Keep full conversation ID (including ;messageid= for thread replies)
      id: conversationId,
...
export async function sendMSTeamsMessages(params: {
  ...
  // Always use proactive messaging (continueConversation) for reliability.
  // The inline ctx.sendActivity() path fails silently when the HTTP webhook context
  // closes (e.g., agent takes >15s). Proactive sends work regardless of context lifecycle.
  // For thread replies, the conversationRef.conversation.id already includes ;messageid=
  // which tells Teams to send into the specific thread.
